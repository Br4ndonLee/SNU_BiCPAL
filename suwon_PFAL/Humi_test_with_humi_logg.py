...
class SensorDataLogger:
    ...
    def setup_csv_file(self):
        ...
        # self.f = open(str(now) + 'env.csv', 'a', newline='')
        # self.wr = csv.writer(self.f)
        
        now = datetime.datetime.now()
        file_name = 'humi_cntl_' + str(now) + 'env.csv'
...
if __name__ == "__main__":
    ...
    try:
        while True:
            ...
            # Control the humidifier based on humidity conditions
            if 2 < hum_int < 70:
                humidifier.turn_on()
            elif hum_int >= 85:
                humidifier.turn_off()
