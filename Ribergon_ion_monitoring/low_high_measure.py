...
def low_measurement():
	...
	# for eliminate capacitance's stored current
	os.system(EXPORT101)
...
def sample_measurement():
	...
	os.system(EXPORT301)
	first_start = time.time()
	for i in range(0,5): # edit 5 to 10
		os.system(EXPORT302)
	last_end = time.time()
...
def rinse():
	...
	GPIO.output(low, GPIO.HIGH); time.sleep(30)

def rinse_using_sample():
	GPIO.output(sample, GPIO.LOW); time.sleep(rinse_t)
	GPIO.output(sample, GPIO.HIGH); time.sleep(30)
